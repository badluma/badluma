## About me

Hello, I'm badluma, and I am interested in coding and AI (not vibe coding). My main language is Python, but I want to learn languages like C, C++, and Rust. I'm currently working on getting better at JavaScript with a few small projects that I'm not uploading to GitHub yet.

I got interested in programming by making Scratch games in third grade. A few years later, Unity Engine caught my attention, and I tried to make my own game inside of it. However, I soon realized that it was too much for me to make a whole game with graphics, sounds, animations, and more. When I learned that the game industry is apparently horrible to work at, I started developing simple scripts and terminal-based games in Python.

Today, I'm way more experienced in Python than before, but I am still not a pro and still learning stuff. I also developed a small homebrew application for the Nintendo 3DS in C++, and also some small websites.
```toml
[general]
age = 16
gender = "male"
languages = ["german, english"]
programming_languages = ["python", "rust"]

[usernames]
discord = "badluma"
instagram = "badlumaa"
mastodon = "badluma"
youtube = "badluma"
pnid = "badlumaa"
minecraft = "badluma"

[hardware]
device = "Macbook Air"
chip = "Apple M4"
memory = "16GB"

[software]
ide = "zed.dev"
video_editing = "Adobe Premiere Pro"
documentation = "Obsidian"
```


